  ...
  <div>
    ...
    >
      ...
      <div>
        ...
        分配新的角色：<el-select v-model="selectedRoleId" placeholder="请选择">
          <el-option
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="setRole(selectedRoleId)">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 展示分配权限对话框信息
    showSetRoleDialog: async function (userInfo) {
      this.userInfo = userInfo
      // 获取下拉框角色列表
      const { data: res } = await this.$http.get('roles')
      if (res.meta.status !== 200) {
        return this.$http.message('分配权限失败')
      }
      this.roleList = res.data
      this.setRoleDialogVisible = true
    },
    // 给用户分配角色
    setRole: async function (selectedRoleId) {
      const { data: res } = await this.$http.put(`users/${this.userInfo.id}/role`, { rid: selectedRoleId })
      if (res.meta.status !== 200) {
        this.$http.message.error('分配角色失败')
      }
      this.getUserList()
